    ...
    (flet ((native-size (x)
      ...
      (macrolet ((sql-type-case (&body entries)
        ...
        (sql-type-case ("int2" (make-instance 'sql-integer-type :bit-size 16))
                       ...
                       ("char" (make-instance 'sql-character-type :size (native-size (third* description))))
                       ("bpchar" (make-instance 'sql-character-type :size (native-size (third* description))))
                       ("varchar" (make-instance 'sql-character-varying-type
                                                 :size (native-size (third* description))))
